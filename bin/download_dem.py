...
GMTED_BASE_URL = ('https://edcintl.cr.usgs.gov/downloads/sciweb1/shared/topo/downloads/GMTED/Global_tiles_GMTED/'
...
def main(directory: Path = None, stat: str = 'mean', arcsec: float = 15, overwrite=False, sleep=1.0,
    ...
    print(f'Starting download of stat=[{stat}], arcsec=[{arcsec}], lon=[{lon_min}, {lon_max}],'
    ...
    print(f'Completed download of [{len(files)}] files for stat=[{stat}], arcsec=[{arcsec}],'
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Download DEM GeoTIFFs from the USGS.'
    )
    parser.add_argument(
        '-d', '--directory',
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help='Number of seconds to sleep between downloads (default=1).'
    )
