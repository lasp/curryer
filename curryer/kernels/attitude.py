...
from .classes import AbstractKernelProperties, AbstractKernelWriter, TypedDataDescriptor
...
class AbstractAttitudeProperties(AbstractKernelProperties):
    ...
    input_data_columns: list[str] = None  # field(default_factory=lambda: [])
    input_rate_columns: list[str] = field(default_factory=lambda: ["rate_x", "rate_y", "rate_z"])
    ...
    def __post_init__(self):
        ...
        if not (
            isinstance(self.input_body, spicierpy.obj.AbstractObj) and isinstance(self.input_frame, spicierpy.obj.Frame)
        ):
            loaded_krns = None
    ...
    def to_dict(self):
        ...
        if self.input_body.is_frame:
            ...
            logger.info(
                "Changing `input_body` (INSTRUMENT_ID) from [%s] to [%s], as the former is believed to be a"
                " spacecraft ID. The msopck program assumes `clock_id = input_id // 1000` and"
                " `clock_id == spacecraft_id`",
                self.input_body.id,
                self.input_body.frame.id,
            )
...
class AttitudeQuaternionProperties(AbstractAttitudeProperties):
    ...
    SUPPORTED_INPUT_DATA_TYPES: typing.ClassVar = (
        AttitudeInputDataTypes.SPICE_QUAT,
        AttitudeInputDataTypes.MSOP_QUAT,
        AttitudeInputDataTypes.FLIPSIGN_QUAT,
    )
...
class AttitudeEulerProperties(AbstractAttitudeProperties):
    ...
    input_rotations_order: list[str] = field(default_factory=lambda: ["X", "Y", "Z"])
    input_rotations_type: str = "SPACE"  # or 'BODY' (SPACE= ax * ay * az)
    input_angle_units: str = "DEGREES"  # or 'RADIANS'
    input_data_columns: list[str] = field(default_factory=lambda: ["angle_x", "angle_y", "angle_z"])
    ...
    def prepare_input_data(self, input_data: pd.DataFrame) -> list[pd.DataFrame]:
        ...
        if "QUAT" in self.properties.input_data_type.name:
            ...
            if n_invalid > 0:
                ...
                logger.warning(
                    "Removing [%d/%d] quaternions that are invalid (zeros) with min/max range=[%s, %s]",
                    n_invalid,
                    is_invalid.size,
                    dt_min_bad,
                    dt_max_bad,
