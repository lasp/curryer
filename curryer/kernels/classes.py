...
class AbstractKernelWriter(metaclass=ABCMeta):
    ...
    def _chunk_table_by_gaps(self, table: pd.DataFrame, index_dt_type="utc"):
        ...
        if idx_gaps.size:
            logger.info(
                "Kernel input data has [%d] gaps larger than [%s], splitting write into [%d] segments",
                idx_gaps.size,
                self.properties.input_gap_threshold,
                idx_gaps.size + 1,
            )
            idx_gaps += 1
            ...
            while ith <= idx_gaps.size:
                ...
                if logger.isEnabledFor(logging.DEBUG):
                    logger.debug(
                        "Input chunk [%d/%d] with [%d] values spanning [%s, %s]",
                        ith + 1,
                        idx_gaps.size + 1,
                        chunks[-1].shape[0],
                        chunks[-1].index[0],
    ...
    def prepare_setup_config(self):
        ...
        # none was provided.
        return write_setup(
            None, self.KTYPE, config, mappings=self.properties.mappings, validate=True, parent_dir=self.parent_dir
        )
    ...
    def _safe_write_setup_config(self, setup_txt):
        ...
        # Defined file is an invalid type.
            ...
            raise AttributeError(f"`setup_file` must be a str or None, not: {self.setup_file!r}")
