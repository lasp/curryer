...
from . import attitude, classes, ephemeris
...
class KernelCreator:
    ...
    def write_from_json(
        self, properties_file: Path, output_kernel: Path = None, input_data=None, input_columns=None, overrides=None
    ):
        ...
        writer(input_data, output_kernel, overwrite=self._overwrite, append=self._append)
        logger.info(
            "Completed kernel [%s] for file: %s from %s",
            "APPEND" if self._append else "WRITE",
            output_kernel,
            properties_file,
        )
        return output_kernel
