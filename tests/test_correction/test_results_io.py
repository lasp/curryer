...
def test_build_netcdf_structure_shapes(clarreo_cfg):
    ...
    nc = correction._build_netcdf_structure(setup, sweep, netcdf_config, n_param_sets=3, n_gcp_pairs=2)
...
def test_build_netcdf_structure_zero_initialised(clarreo_cfg):
    ...
    setup, sweep, _output, netcdf_config = clarreo_cfg
    nc = correction._build_netcdf_structure(setup, sweep, netcdf_config, n_param_sets=2, n_gcp_pairs=1)
...
def test_checkpoint_save_load_cleanup(clarreo_cfg, tmp_path):
    ...
    nc = correction._build_netcdf_structure(setup, sweep, netcdf_config, n_param_sets=2, n_gcp_pairs=2)
    nc["rms_error_m"][0, 0] = 100.0
    nc["rms_error_m"][1, 0] = 150.0

    correction._save_netcdf_checkpoint(nc, output_file, setup, sweep, netcdf_config, pair_idx_completed=0)
    ckpt = output_file.parent / f"{output_file.stem}_checkpoint.nc"
    assert ckpt.exists()

    loaded, completed = correction._load_checkpoint(output_file)
