"""Tests for ``curryer.correction.image_match``.

Covers ``integrated_image_match`` against 20 CLARREO test cases (5 scenes ×
4 sub-cases each: unbinned a/b, 3-pixel-binned c/d).
"""
...
def image_grid_from_struct(mat_struct) -> ImageGrid:
    return ImageGrid(
...
def apply_geolocation_error(subimage: ImageGrid, gcp: ImageGrid, lat_error_km: float, lon_error_km: float) -> ImageGrid:
    """Return a copy of *subimage* with an imposed lat/lon geolocation error."""
    mid_lat = float(gcp.lat[gcp.lat.shape[0] // 2, gcp.lat.shape[1] // 2])
    lat_off, lon_off = great_circle_displacement_deg(lat_error_km, lon_error_km, mid_lat)
    return ImageGrid(
        data=subimage.data.copy(),
        lat=subimage.lat + lat_off,
        lon=subimage.lon + lon_off,
...
class TestImageMatch:
    """Validates ``integrated_image_match`` against 20 known test cases."""

    @pytest.fixture(autouse=True)
    def _setup(self, image_match_dir):
        self.test_dir = image_match_dir

    def _run(self, subimg_file, gcp_file, ancil_file, psf_file, pix_vec_file, lat_lon_err):
        subimage = image_grid_from_struct(loadmat(subimg_file, squeeze_me=True, struct_as_record=False)["subimage"])
        gcp = image_grid_from_struct(loadmat(gcp_file, squeeze_me=True, struct_as_record=False)["GCP"])
        ...
        logger.info(
            "case=%s  lat=%+.3f exp=%+.3f  lon=%+.3f exp=%+.3f  ccv=%.4f  row=%d  col=%d",
            subimg_file.name,
            result.lat_error_km,
            lat_lon_err[0],
            result.lon_error_km,
            lat_lon_err[1],
            result.ccv_final,
            result.final_index_row,
            result.final_index_col,
        )
        return result
    ...
    def test_case_1a_unbinned(self):
        ...
        np.testing.assert_allclose(r.lat_error_km, lat_lon_err[0], atol=0.05)
        np.testing.assert_allclose(r.lon_error_km, lat_lon_err[1], atol=0.05)
        np.testing.assert_allclose(r.ccv_final, 1.0, atol=0.01)
        np.testing.assert_allclose(r.final_index_row, 23)
    ...
    def test_case_1b_unbinned(self):
        ...
        np.testing.assert_allclose(r.lon_error_km, lat_lon_err[1], atol=0.05)
        np.testing.assert_allclose(r.ccv_final, 1.0, atol=0.01)
        np.testing.assert_allclose(r.final_index_row, 21)
        np.testing.assert_allclose(r.final_index_col, 22)
    ...
    def test_case_1c_binned(self):
        ...
        np.testing.assert_allclose(r.ccv_final, 1.0, atol=0.01)
        np.testing.assert_allclose(r.final_index_row, 21)
        np.testing.assert_allclose(r.final_index_col, 23)
    ...
    def test_case_1d_binned(self):
        ...
        np.testing.assert_allclose(r.ccv_final, 1.0, atol=0.01)
        np.testing.assert_allclose(r.final_index_row, 23)
        np.testing.assert_allclose(r.final_index_col, 21)
    ...
    def test_case_3c_binned(self):
        ...
        np.testing.assert_allclose(r.lat_error_km, lat_lon_err[0], atol=0.07)
        np.testing.assert_allclose(r.lon_error_km, lat_lon_err[1], atol=0.07)
        np.testing.assert_allclose(r.ccv_final, 1.0, atol=0.01)
        np.testing.assert_allclose(r.final_index_row, 22)
        np.testing.assert_allclose(r.final_index_col, 23)
    ...
    def test_case_3d_binned(self):
        ...
        np.testing.assert_allclose(r.ccv_final, 1.0, atol=0.01)
        np.testing.assert_allclose(r.final_index_row, 23)
        np.testing.assert_allclose(r.final_index_col, 20)
