...
from curryer.spicetime import utils, convert, native
...
class SpicetimeUtilsTestCase(unittest.TestCase):
    ...
    def test_find_mapping_path_realish(self):
        gpsd_func = utils.InputAsArray(
            np.float64, False, defaults=dict(to_epoch='GPS')
        ...
        self.assertListEqual(
            [convert.from_utc, convert.to_ugps, native.ugps_to_gps_fraction, gpsd_func],
            conversions
        )

        utc_times = np.array(['2020-01-01', '1980-01-06', '1981-01-06'])
        gpsd_times = utils.apply_conversions(conversions, utc_times)
        npt.assert_allclose(np.array([14605., 0., 366.]), gpsd_times)

        self.assertEqual(
            '2020-10-01T18:44:57.167939',
            utils.apply_conversions(utils.find_mapping_path(convert.conversions, 'ugps', 'iso'), 1285613115167939,
