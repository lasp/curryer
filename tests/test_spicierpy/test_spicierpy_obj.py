...
class ObjTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self._loaded_kernels = spicierpy.ext.load_kernel([self.kernels])
    ...
    def test_obj_spacecraft_specified_attr(self):
        ...
        sc = obj.Spacecraft(
            -9999, frame=frame, clock=clock, ephemeris=ephemeris, attitude=attitude, instruments=[instrument]
        )
    ...
    def test_obj_instrument(self):
        for id_or_name in [-125544101, "tsis_tim_glint"]:
            with self.subTest(arg=id_or_name):
                ...
                self.assertIsNotNone(re.search(r"^Instrument\(.+\)$\s+Frame\(.+\)$", txt, re.MULTILINE))
