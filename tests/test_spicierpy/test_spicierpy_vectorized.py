...
from curryer.spicierpy import vectorized
...
class VectorizedTestCase(unittest.TestCase):
    ...
    def test_spkezr_original_behavior(self):
        ...
        with ext.load_kernel(self.kernels):
            ...
            arr, _ = spiceypy.spkezr(self.target_name, self.et_times[0], obs=self.observer_name,
                                     abcorr=self.correction, ref=self.reference_frame)
            self.assertTupleEqual((6,), arr.shape)

            # It _now_ supports time array (>1 value)!
            out = spiceypy.spkezr(self.target_name, self.et_times, obs=self.observer_name,
                                  abcorr=self.correction, ref=self.reference_frame)
            ...
            with self.assertRaises(SpiceyError) as raised:
                spiceypy.spkezr(self.target_id, self.et_times[0], obs=self.observer_name,
                                abcorr=self.correction, ref=self.reference_frame)
            self.assertEqual(raised.exception.short, 'SPICE(EMPTYSTRING)')

            # Doesn't support observer id (int).
            with self.assertRaises(SpiceyError) as raised:
                spiceypy.spkezr(self.target_name, self.et_times[0], obs=self.observer_id,
                                abcorr=self.correction, ref=self.reference_frame)
    ...
    def test_spkezr_vectorized_variant_improved_api(self):
        with ext.load_kernel(self.kernels):
            # Correct call - Scalar time value, target string, observer string.
            arr, _ = vectorized.spkezr(self.target_name, self.et_times[0], obs=self.observer_name,
                                       abcorr=self.correction, ref=self.reference_frame)
            self.assertTupleEqual((6,), arr.shape)

            # Support arrays!
            arr, _ = vectorized.spkezr(self.target_name, self.et_times, obs=self.observer_name,
                                       abcorr=self.correction, ref=self.reference_frame)
            self.assertTupleEqual((4, 6), arr.shape)

            # Supports ID values (ints)!
            arr, _ = vectorized.spkezr(self.target_id, self.et_times[0], obs=self.observer_id,
                                       abcorr=self.correction, ref=self.reference_frame)
            self.assertTupleEqual((6,), arr.shape)
    ...
    def test_spkezp_original_behavior(self):
        ...
        with ext.load_kernel(self.kernels):
            ...
            arr, _ = spiceypy.spkezp(self.target_id, self.et_times[0], obs=self.observer_id,
                                     abcorr=self.correction, ref=self.reference_frame)
            self.assertTupleEqual((3,), arr.shape)

            # Doesn't support time array (>1 value).
            with self.assertRaises(TypeError) as raised:
                spiceypy.spkezp(self.target_id, self.et_times, obs=self.observer_id,
                                abcorr=self.correction, ref=self.reference_frame)
            self.assertIn('only length-1 arrays can be converted', raised.exception.args[0])

            # Doesn't support target name (string).
            with self.assertRaises(TypeError) as raised:
                spiceypy.spkezp(self.target_name, self.et_times[0], obs=self.observer_id,
                                abcorr=self.correction, ref=self.reference_frame)
            self.assertIn('integer', raised.exception.args[0])

            # Doesn't support observer name (string).
            with self.assertRaises(TypeError) as raised:
                spiceypy.spkezp(self.target_id, self.et_times[0], obs=self.observer_name,
                                abcorr=self.correction, ref=self.reference_frame)
            self.assertIn('integer', raised.exception.args[0])
    ...
    def test_spkezp_vectorized_variant_improved_api(self):
        with ext.load_kernel(self.kernels):
            ...
            arr, _ = vectorized.spkezp(self.target_id, self.et_times[0], obs=self.observer_id,
                                       abcorr=self.correction, ref=self.reference_frame)
            self.assertTupleEqual((3,), arr.shape)

            # Supports arrays!
            arr, _ = vectorized.spkezp(self.target_id, self.et_times, obs=self.observer_id,
                                       abcorr=self.correction, ref=self.reference_frame)
            self.assertTupleEqual((4, 3), arr.shape)

            # Supports name values (str)!
            arr, _ = vectorized.spkezp(self.target_name, self.et_times[0], obs=self.observer_name,
                                       abcorr=self.correction, ref=self.reference_frame)
            self.assertTupleEqual((3,), arr.shape)
