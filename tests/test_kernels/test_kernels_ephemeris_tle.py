    ...
    def test_ctim_tle_kernel(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertLessEqual(
                np.abs(tle_input_data.index.min() - np.timedelta64(12, "h") - span[0]), np.timedelta64(1, "ms")
            )
            self.assertLessEqual(
                np.abs(tle_input_data.index.max() + np.timedelta64(12, "h") - span[1]), np.timedelta64(1, "ms")
            )
