...
class WriterSpkTestCase(unittest.TestCase):
    ...
    def test_write_setup_return_str(self):
        txt = writer.write_setup(setup_file=None, template="spk", mappings=self.naif_body, configs=self.spk_configs)
        self.assertIn("NAIF_BODY_CODE += ( -999 )", txt)
        self.assertIn("NAIF_BODY_NAME += ( 'spk_test' )", txt)

    def test_write_setup_write_to_obj(self):
        setup_file = StringIO()
        r = writer.write_setup(setup_file=setup_file, template="spk", mappings=self.naif_body, configs=self.spk_configs)
        setup_file.seek(0)
        txt = setup_file.read()
...
class WriterCkTestCase(unittest.TestCase):
    ...
    def test_write_setup_return_str(self):
        txt = writer.write_setup(setup_file=None, template="ck", mappings=self.naif_body, configs=self.ck_configs)
        self.assertIn("NAIF_BODY_CODE += ( -999 )", txt)
