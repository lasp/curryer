    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        for kernel_config in kernel_configs:
            generated_kernels.append(creator.write_from_json(
                kernel_config, output_kernel=tmp_dir, input_data=input_file_or_obj,
            ))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Generate L1A geolocation data product.'
    )
    parser.add_argument(
        '--meta_kernel',
        type=str,
        help='Meta kernel json file for the mission.'
    )
    parser.add_argument(
        '--generic_dir',
        type=str,
        help='Directory containing the generic SPICE kernels.'
    )
    parser.add_argument(
        '--kernel_configs',
        type=str,
        nargs='+',
        help='One or more kernel configuration properties files (json).'
    )
    parser.add_argument(
        '--output_dir',
        type=str,
    ...
    parser.add_argument(
        '--input_file_or_obj',
        type=str,
        help='Input telemetry file to create kernels from.'
    )
    parser.add_argument(
        '-t', '--time_range',
        type=str,
