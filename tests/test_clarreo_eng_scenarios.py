...
from curryer.compute import constants, elevation, pointing, spatial
...
class ClarreoEngScenariosTestCase(unittest.TestCase):
    ...
    def test_case1_kernels_files_high_level(self):
        ...
        generated_kernels.append(
            creator.write_from_json(
                self.test_dir / "iss_sc_v01.ephemeris.spk.testcase1.json",
        ...
        generated_kernels.append(
            creator.write_from_json(self.test_dir / "iss_sc_v01.attitude.ck.testcase1.json", output_kernel=self.tmp_dir)
        )
        ...
        with sp.ext.load_kernel([mkrn.sds_kernels, mkrn.mission_kernels, generated_kernels]):
            ...
            logger.info(
                "Geoloc+Dataset process time=[%s], count=[%d], granule_est_time=[%s]",
                t1 - t0,
                ugps_times.size,
                (t1 - t0) / ugps_times.size * 4500,
            )
            ...
            lonlats_arr = np.stack(
                [lonlats_ds["longitude_ellipsoidal"].values, lonlats_ds["latitude_ellipsoidal"].values], axis=1
            )
    ...
    def test_case1_kernels_files_tlm_input(self):
        ...
        generated_kernels.append(
            creator.write_from_json(
                self.data_dir / "cprs_az_v01.attitude.ck.json",
                output_kernel=self.tmp_dir,
                input_data=tlm_dataset,
            )
        )
        generated_kernels.append(
            creator.write_from_json(
                self.data_dir / "cprs_el_v01.attitude.ck.json",
                output_kernel=self.tmp_dir,
                input_data=tlm_dataset,
            )
        )
        ...
        with sp.ext.load_kernel([mkrn.sds_kernels, mkrn.mission_kernels, generated_kernels]):
            ...
            lonlats = np.stack(
                [lonlats["longitude_ellipsoidal"].values, lonlats["latitude_ellipsoidal"].values], axis=1
            )
            ...
            self.assertEqual(
                qf_counts[
                    (
                        SQF.CALC_ANCIL_NOT_FINITE
                        | SQF.CALC_ANCIL_INSUFF_DATA
                        | SQF.CALC_ELLIPS_INSUFF_DATA
                        | SQF.SPICE_ERR_MISSING_ATTITUDE
                    ).value
                ],
                7200,
            )
    ...
    def build_prod_tlm_file():
        ...
        dataset = xr.Dataset(
            ...
            attrs={
                ...
                "source": (
                    "GeolocationTestCase1.xlsx, SPICE conversions from ECEF to ECI"
    ...
    def test_case1_kernels_files_low_level(self):
        ...
        with sp.ext.load_kernel([mkrn.sds_kernels, mkrn.mission_kernels, generated_kernels]):
            ...
            t1 = pd.Timestamp.utcnow()
            logger.info(
                "Geolocation process time=[%s], count=[%d], granule_est_time=[%s]",
                t1 - t0,
                ugps_times.size,
                (t1 - t0) / ugps_times.size * 4500,
            )
            ...
            logger.info(
                "Terrain-correction process time=[%s], shape=[%d], granule_est_time=[%s]",
                t3 - t2,
                ugps_times.size,
                (t3 - t2) / ugps_times.size * 4500,
            )
    ...
    def test_case1_kernels_manual(self):
        ...
        with sp.ext.load_kernel([mkrn.sds_kernels, mkrn.mission_kernels, generated_kernels]):
            ...
            logger.info(
                "Geolocation (expected, computed, error):\n\tLon: %10.6f, %10.6f (%.6f)\n\tLat: %10.6f, %10.6f (%.6f)",
                expt_lon,
                calc_lon,
                err_lon,
                expt_lat,
